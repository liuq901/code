...
void work()
{
   void init(int),build(),floodfill(int),change(int,int),negate(int,int);
...
void negate(int x,int root)
{
   ...
   while (x!=root)
   {
      ...
      int p=h[father[x]][0],q=h[root][0];
      if (p==q)
      {
         update(h[p][6],h[root][1],h[father[x]][1]);
         break;
      }
      update(h[p][6],1,h[father[x]][1]);
      x=h[p][2];
   }
}
void update(int x,int l,int r)
{
   ...
   if (r<=mid)
      update(tree[x].left,l,r);
   else if (l>mid)
      update(tree[x].right,l,r);
   else
   {
      update(tree[x].left,l,mid);
      update(tree[x].right,mid+1,r);
   }
   if (tree[tree[x].left].x>tree[tree[x].right].x)
      tree[x].x=tree[tree[x].left].x;
   else
