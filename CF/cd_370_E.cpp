...
void work(int n)
{
    ...
    for (int i=2;i<=n;i++)
    {
        l[i]=l[i-1].second>=2?make_pair(l[i-1].first+1,1):make_pair(l[i-1].first,l[i-1].second+1);
        r[i]=r[i-1].second==5?make_pair(r[i-1].first+1,1):make_pair(r[i-1].first,r[i-1].second+1);
        if (a[i]!=0)
        {
